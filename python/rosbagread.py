...
if(sys.version_info < (3,0)):
	...
	# print('Using python2 version of pickle')
else:
	import pickle
	# print('Using python3 version of pickle')


import rosbag
from roslib.message import get_message_class

def bag_to_dataframe(bag_name, include=None, exclude=None, parse_header=False, seconds=True):
    '''
    ...
    if(sys.version_info > (3,0)):
        yaml_info = get_bag_info_python3(bag_name)
    else:
        yaml_info = get_bag_info_python2(bag_name)
    ...
    datastore['header']=index
    dataframe=pd.DataFrame(data=datastore)
    #dataframe.index.name='header'
    colnames = list(dataframe.columns.values)
    if 'header' in colnames:
        notHeaderCols= [name for name in colnames if name is not 'header']
        sortednames=['header']+notHeaderCols
        dataframe=dataframe[sortednames]
    return dataframe
...
def prune_topics(bag_topics, include, exclude):
    ...
    if include is None:
        ...
        try:
            ...
            logging.warn('Error in topic selection Using All!')
            topics_to_use = set()
            for t in bag_topics:
                topics_to_use.add(t)
        ...
        for info in topic_info:
            if info['topic'] == topic:
                ...
                if msg_class is None:
                    logging.warn('Could not find types for ' + topic + ' skipping ')
                else:
...
def get_bag_info_python3(bag_file):
...
def get_bag_info_python2(bag_file):
    '''Get uamle dict of the bag information
    by calling the subprocess -- used to create correct sized
    arrays'''
    # Get the info on the bag
...
def rosbagread(bagfile,ignore=None,read=None):
    # Read a bag file and generate a dictionary of topics containing data in pandas dataframes
    # get list of topics to parse
    ##### make get_bag_info check what version of python you are using #####
    # print(sys.version_info)
    if(sys.version_info > (3,0)):
        yaml_info = get_bag_info_python3(bagfile)
    else:
        yaml_info = get_bag_info_python2(bagfile)

    bag_topics = get_topics(yaml_info)
    bag_topics = prune_topics(bag_topics,read,ignore)
    bagdata = dict()
    for topic in bag_topics:
        print('Processing ' + topic)
        bagdata[topic] = bag_to_dataframe(bagfile, include=topic)
    return bagdata
